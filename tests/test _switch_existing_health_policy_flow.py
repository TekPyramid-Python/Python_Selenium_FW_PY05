...
class Testawitchtoexcistinhealthpolicy(BaseTest):
    ...
    def test_super_top_up_navigation(self):
        ...
        with allure.step("Verify successful 'Health Insurance Policy' page title"):
            ...
            element = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(policy.existing_policy_link))
            self.driver.execute_script("arguments[0].click();", element)

        expected_popup_text = 'Switch Existing health policy'
        actual_popup_text = policy.text_excicting_plan_popup()
        assert expected_popup_text == actual_popup_text, f"Expected pop up text '{expected_popup_text}', but got '{actual_popup_text}'"
        policy.send_existing_policy_mobile_number()
        policy.click__existing_policy_on_get_a_call_back()

        expected_popup_confirmation_text = 'We will reach out to you shortly'
        actual_popup_confirmation_text = policy.existing_policy_confirmation_message
        ...
        policy.click_on_ok_button_existing_policy()
